...
void FinalProject::printMovieDirector(){
    printMovieDirector(root); //display all movies in bst
}
...
void FinalProject::DeleteAll(MovieNode *node){
    if(node == NULL){
        cout<<"Empty"<<endl;
    }

    if(node->leftChild !=NULL) //if leftchild of the node is not empty, then delete the node and recall function by using leftchildnode
    {
        DeleteAll(node->leftChild);
    }
    if(node->rightChild !=NULL)//if rightchild of the node is not empty, then delete the node and recall function by using rightchildnode
    {
        DeleteAll(node->rightChild);
    }

cout<<"Deleting: "<<node->title<<endl; //display each node while deleting
